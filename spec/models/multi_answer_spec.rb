...
RSpec.describe MultiAnswer, type: :model do
  ...
  describe '.result' do
    ...
    let!(:ruby_answers)   { create_list(:multi_answer, 3, question: question, choice: ruby) }
    let!(:python_answers) { create_list(:multi_answer, 2, question: question, choice: python) }
    let!(:expected)       { [ {ruby.id => 0.6}, { python.id => 0.4}] }
    
    it "returns an array of ruby_answers" do
